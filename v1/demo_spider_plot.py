...
for scale_plot in [True, False][:]:
    spider_plot = SpiderPlot('Classes', autodraw=False, scale_plot=scale_plot, figname=f'SpiderPlot_{scale_plot}')

    for category in categories:
        tick_values = linstring(min_tick, max_tick + random() * 0.25, N_tick)
    ...
    data = [d1, d2, d3]
    classes = ['Warrior', 'Wizard', 'Ranger']

    for color, d, c in zip(colors, data, classes):
        spider_plot.add_data(c, d, color)
