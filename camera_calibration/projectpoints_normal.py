...
def mix(a, b):
    ao = [a_ for a_ in a for b_ in b]
    bo = [b_ for a_ in a for b_ in b]
...
a1s, a2s = mix((-out_fov[0]/2, out_fov[0]/2), (-out_fov[1]/2, out_fov[1]/2))
...
# Center points
x, y = cameraMatrix[0][2] * scale[0], cameraMatrix[1][2] * scale[1]
...
for i, point in enumerate(undistorted_points):
    undistorted_img = cv2.circle(undistorted_img, (round(point[0][0]),round(point[0][1])), 1, ((1-i/len(image_points))*255,0,i*255/len(image_points)), thickness=5, lineType=8)
